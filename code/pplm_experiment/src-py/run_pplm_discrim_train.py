...
np.random.seed(0)
EPSILON = 1e-10
example_sentence = "This goes against our traditions and our constitution"
max_length_seq = 100
...
def train_discriminator(
    ...
    if dataset == "SST":
        ...
        classes = set()
        with open(dataset_fp, encoding='latin1') as f:
    ...
    for epoch in range(epochs):
        ...
        if save_model:
            ...
            torch.save(
                ...
                "{}/{}_classifier_head_epoch_{}.pt".format(model_path, dataset, epoch + 1),
            )


def prepare_ds(dataset_fp, discriminator, device, batch_size=16, max_length_seq=500):
    x = []
    y = []
    ...
    dataset = Dataset(x, y)
    loader = get_cached_data_loader(dataset, batch_size, discriminator, device=device)

    return dataset, loader


def load_predictor(pretrained_model_path, head_saved_weights, classes, device):
    idx2class = sorted(classes)
    discriminator = Discriminator(
                        class_size=len(idx2class), 
                        pretrained_model=pretrained_model_path, cached_mode=True,
                        device=device).to(device)

    discriminator.classifier_head.load_state_dict(torch.load(head_saved_weights))
    

    def predict(input_sentence):
        input_t = discriminator.tokenizer.encode(input_sentence)
        input_t = torch.tensor([input_t], dtype=torch.long, device=device)
        input_t = discriminator.avg_representation(input_t)

        log_probs = discriminator(input_t).data.cpu().numpy().flatten().tolist()
        
        class_props = [(c, math.exp(log_prob)) for c, log_prob in zip(classes, log_probs)]

        return sorted(class_props, key=lambda x: -x[1])[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default='./', help="whether to save the model")
